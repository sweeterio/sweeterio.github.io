...
### 1. 打开/OPEN
  ...
  打开操作，一般要在测试数据中指定标签页名称，如：#tab_name=百度。

| 操作   | 元素     | 测试数据  |
| ------ | ------- | --------- |
| `打开` | 百度首页 | #tab=百度 |

  如果需要清理缓存再打开，则在测试数据中写上：`#clear=Y`

| 操作 | 元素     | 测试数据              |
| --- | -------- | --------------------- |
| 打开 | 百度首页 | #tab=百度，`#clear=Y` |
...
- `title` 和 `current_url` 直接在测试数据中写预期结果即可。
...
如果输入是具有联想功能的下拉菜单时，可能需要逐个字符输入功能（模拟真实的键盘输入场景），应写为 `char=...`,如：
...
如果不想做清空文本操作，需在测试数据中，加上：`#clear=N`，如下:
...
| 操作   | 元素          | 测试数据 |
| ------ | ------------- | ------- |
| `执行` | `SNIPPET_001` | kw=搜狗  |

把变量 `kw` 赋值为 `搜狗`，此步骤会执行用例片段 `SNIPPET_001`，其搜索的关键字为 `搜狗`。
...
| ----- | ------- | ---------------------- |---- | ------ |
| baidu | js点击 | `$("a.xxx")[0].click()` | js | frame1 |
